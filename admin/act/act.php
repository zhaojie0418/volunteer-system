...
// 分页显示信息
$page_size = 6;
$res = mysqli_query($link, 'select count(*) from `act_info`');
...
//读取数据库，查询结果
$sql = "select * from (select * from `act_info` $limit) a $where";
...
if (isset($_POST['keyword']) && empty($_POST['state'])) {
    ...
    $where = "where act_name like '%$keyword%'";
    $page_size = 6;
    $res = mysqli_query($link, "select count(*) from `act_info` where act_name like '%$keyword%'");
    $count = mysqli_fetch_row($res);
    $count = $count[0];
    $max_page = ceil($count / $page_size); //向上取整
    $page = isset($_GET['page']) ? intval($_GET['page']) : 1;
...
if (empty($_POST['keyword']) && isset($_POST['state'])) {
    if ($_POST['state'] == 'check') {
        ...
        $page_size = 6;
        $res = mysqli_query($link, "select count(*) from `act_info` where `state`='待审核'");
        $count = mysqli_fetch_row($res);
        $count = $count[0];
        $max_page = ceil($count / $page_size); //向上取整
        $page = isset($_GET['page']) ? intval($_GET['page']) : 1;
        ...
        $page_size = 6;
        $res = mysqli_query($link, "select count(*) from `act_info` where `state`='已通过'");
        $count = mysqli_fetch_row($res);
        $count = $count[0];
        $max_page = ceil($count / $page_size); //向上取整
        $page = isset($_GET['page']) ? intval($_GET['page']) : 1;
...
if (isset($_POST['keyword']) && isset($_POST['state'])) {
    ...
    $keyword = mysqli_real_escape_string($link, $keyword);
    if ($_POST['state'] == 'check') {
        $where = "where `state`='待审核' and act_name like '%$keyword%'";
        $page_size = 6;
        $res = mysqli_query($link, "select count(*) from `act_info` where `state`='待审核' and act_name like '%$keyword%'");
        $count = mysqli_fetch_row($res);
        $count = $count[0];
        $max_page = ceil($count / $page_size); //向上取整
        $page = isset($_GET['page']) ? intval($_GET['page']) : 1;
        ...
        $where = "where `state`='已通过' and act_name like '%$keyword%'";
        $page_size = 6;
        $res = mysqli_query($link, "select count(*) from `act_info` where `state`='已通过' and act_name like '%$keyword%'");
        $count = mysqli_fetch_row($res);
        $count = $count[0];
        $max_page = ceil($count / $page_size); //向上取整
        $page = isset($_GET['page']) ? intval($_GET['page']) : 1;
...
if (isset($_GET['state']) && isset($_GET['id'])) {
    $update = $_GET['state'];
    $act_id = $_GET['id'];
    $sql = "update `act_info` set `state`=$update where `act_id`=$act_id";
    if (query($sql)) {
